 ...
 * @param {string} [p.note] 사람이 읽을 짧은 메모 (파일명, 확정 프레임 수, 품질 경고 등)
 */
export function record({ source, decision, quality, thumb, settings, note }) {
  const entry = {
...
export function toCsv(entries = list()) {
  const head = [
    ...
    'calibrated', 'margin', 'entropy', 'hold_threshold', 'note',
